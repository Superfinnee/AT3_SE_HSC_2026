...
addStationButton.addEventListener("click", () => {
  ...
  input.appendChild(blankStationOption)
  ...
  select.appendChild(blankTrainOption)
...
function updateJourney() {
  ...
  resultsDiv.innerHTML = segments.map(seg => {
    ...
    if (seg.lines.length > 1) {
      selectorHTML = askForLineChoice(seg.lines).outerHTML
    }
    else if (seg.lines.length === 1) {
      selectorHTML = `<input type="hidden" name="lineChoice" value="${seg.lines[0]}">`
    }
...
function askForLineChoice(lines) {
  ...
  chooseLine.required = true
  const defaultOption = document.createElement("option")
  defaultOption.value = ""
  defaultOption.textContent = "Select a line"
  defaultOption.selected = true
  defaultOption.disabled = true
  chooseLine.appendChild(defaultOption)
  for (const line of lines) {
    const option = document.createElement("option")
    option.value = line
    option.textContent = line
    chooseLine.appendChild(option)
  }
  return chooseLine
}